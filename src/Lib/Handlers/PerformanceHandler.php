...
class PerformanceHandler
{
    ...
    protected function setConfigQueryLogState()
    {
        ...
        if (!is_null($this->config->getQueryLogState()))
            return;

        // Set check query log state
        if ($this->config->isQueryLog()) {
            $this->config->setQueryLogState(false);
            ...
            try {
                ...
                $this->config->setQueryLogState(true);
    ...
    {
        if ($this->config->getPresenter() == PresenterConfiguration::PRESENTER_CONSOLE_KEY) {
            $this->presenter = new ConsolePresenter($this->config);
        } elseif ($this->config->getPresenter() == PresenterConfiguration::PRESENTER_WEB_KEY) {
